...
/*
  補助関数群
*/
static void	_skip_spaces(t_lexer *lx)
{
	while (isspace((unsigned char)lx->input[lx->pos]))
		lx->pos++;
}

static t_token	_make_token(t_token_type type, const char *value)
{
	t_token	tok;
...
t_token	*_lexer(const char *input)
{
	...
	while (peek_char(&lx) != '\0')
	{
		if (is_special_char(peek_char(&lx)))
		{
			...
			tokens = token_list_add(tokens, &count, _make_token(ttype, NULL));
		}
		else if (!isspace((unsigned char)peek_char(&lx)))
		{
			...
			if (!w)
			{
				...
				tokens = token_list_add(tokens, &count, _make_token(TT_ERROR,
							NULL));
				break ;
			}
			tokens = token_list_add(tokens, &count, _make_token(TT_CMD, w));
			free(w);
		}
		_skip_spaces(&lx);
	}
